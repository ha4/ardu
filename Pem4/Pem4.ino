...
int get_float(float f, uint16_t *val, uint16_t *tmp)
{
  union {
    float f;
    uint16_t w[2];
...
int get_long(long l, uint16_t *val, uint16_t *tmp)
{
  union {
    uint32_t l;
    uint16_t w[2];
...
float put_float(uint16_t val, uint16_t tmp)
{
  union {
    float f;
    uint16_t w[2];
...
long put_long(uint16_t val, uint16_t tmp)
{
  union {
    uint32_t l;
    uint16_t w[2];
    ...
    *val = adc.command(ch | AD7714::REG_RD | reg, 0xFF);
    ...
    if (hi && addr-1 == prevadr) {
      *val = tempor;
      ...
      if (adc.adc_filth & AD7714::WL24)
            get_long(adc.read24(AD7714::REG_DATA|ch|AD7714::REG_RD), val, &tempor);
      else
            get_long(adc.read16(AD7714::REG_DATA|ch|AD7714::REG_RD), val, &tempor);
    }
    prevadr = addr;
    ...
    if (hi && addr-1 == prevadr) {
      *val = tempor;
      ...
      get_long(adc.read24(reg|ch|AD7714::REG_RD), val, &tempor);
    }
    prevadr = addr;
    ...
    if (hi && addr-1 == prevadr) {
      *val = tempor;
        ...
        adc.fsync(ch);
        adc.wait(ch);
        get_float(adc.read(ch), val, &tempor);
    }
    prevadr = addr;
  } else
    return 0;
...
int reg_adcput(uint16_t addr, uint16_t val)
{
  ...
  if (addr < 300)
    ...
    ch = (subadr >> 1) & 0x7;
    if (hi && addr-1 == prevadr) {
      adc.load24(AD7714::REG_DATA|ch|AD7714::REG_WR, put_long(val, tempor));
    } else {
      tempor=val;
    }
    prevadr = addr;
    ...
    reg = (subadr & 0x2) ? AD7714::REG_GAIN : AD7714::REG_OFFSET;
    if (hi && addr-1 == prevadr) {
      adc.load24(reg|ch|AD7714::REG_WR, put_long(val, tempor));
    } else {
      tempor = val;
    }
    prevadr = addr;
...
//  static uint16_t prevadr= 0;
  ...
  switch(addr) {
  case 0:*val = controller; return 1;
  case 1:*val = ncal; return 1;
  case 2:*val = errorCount; return 1;
  case 3:*val = fsm; return 1;
  case 4: return get_float(regpwm, val, &tempor);
  case 5: *val = tempor; return 1;
  case 6: return get_float(ipart, val, &tempor);
  case 7:*val = tempor; return 1;
  case 8: return get_float((tt-tprev)/1000.0, val, &tempor);
  case 9:*val = tempor; return 1;
  case 10: *val = measured; return 1; /* wait register */
  case 11: *val = measured; return 1;
  case 12: return get_float(0, val, &tempor);
  case 13: *val = tempor; return 1;
  case 14: measured = 0; return get_float(geti, val, &tempor);
  case 15:*val = tempor; return 1;
  case 16: return get_long(millis(), val, &tempor);
  case 17:*val = tempor; return 1;
  case 18: return get_float(fils[0]/CALFILTER, val, &tempor);
  case 19:*val = tempor; return 1;
  case 20: return get_float(fils[1]/CALFILTER, val, &tempor);
  case 21:*val = tempor; return 1;
  default:   return 0;
  }
}
...
//  static uint16_t prevadr= 0;
  ...
  switch(addr) {
  case 0:controller=val; return 1;
  case 1:  return 1;
  case 2:errorCount=val; return 1;
  case 4: tempor = val; return 1;
  case 5: regpwm = put_float(val, tempor); return 1;
  case 6: tempor = val; return 1;
  case 7: ipart  = put_float(val, tempor); return 1;
  default:   return 0;
  }
}
...
int adc_loop()
{
  ...
  case 0:
   ...
   case 2:
    ...
    fsm++;
   case 3:
    adc.conv(adc.read24(AD7714::CHN_12|AD7714::REG_DATA|AD7714::REG_RD));
    fsm++;
   case 4:
    toux = millis();
    adc.fsync(AD7714::CHN_56);
