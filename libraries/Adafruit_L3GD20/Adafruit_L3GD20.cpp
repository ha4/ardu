...
{ 
  ...
  if (_cs == -1) {
    ...
    Wire.write(L3GD20_REGISTER_OUT_TEMP | 0x80);
  ...
  idata.x = word(xhi,xlo);
  idata.y = word(yhi,ylo);
  idata.z = word(zhi,zlo);

  tempr = 25 - tempr; // normalize at 25*C
  
  // Compensate values depending on the resolution
  switch(range)
  {
    case L3DS20_RANGE_250DPS:
      data.x = idata.x*L3GD20_SENSITIVITY_250DPS;
      data.y = idata.y*L3GD20_SENSITIVITY_250DPS;
      data.z = idata.z*L3GD20_SENSITIVITY_250DPS;
      break;
    case L3DS20_RANGE_500DPS:
      data.x = idata.x*L3GD20_SENSITIVITY_500DPS;
      data.y = idata.y*L3GD20_SENSITIVITY_500DPS;
      data.z = idata.z*L3GD20_SENSITIVITY_500DPS;
      break;
    case L3DS20_RANGE_2000DPS:
      data.x = idata.x*L3GD20_SENSITIVITY_2000DPS;
      data.y = idata.y*L3GD20_SENSITIVITY_2000DPS;
      data.z = idata.z*L3GD20_SENSITIVITY_2000DPS;
      break;
  }
  data.x -= offset.x;
  data.y -= offset.y;
...
void Adafruit_L3GD20::calibrate()
{
  int	i;
  int	X,Y,Z;
  float s;
  ...
  switch(range)
  {
    case L3DS20_RANGE_250DPS: s = L3GD20_SENSITIVITY_250DPS;  break;
    case L3DS20_RANGE_500DPS: s = L3GD20_SENSITIVITY_500DPS;  break;
    case L3DS20_RANGE_2000DPS:s = L3GD20_SENSITIVITY_2000DPS; break;
  }
  offset.x=0.02*s*X;
  offset.y=0.02*s*Y;
  offset.z=0.02*s*Z;
